...
## 2026-03-11 - Use native DOM API for continuous stream appending
**Learning:** Using jQuery (`$('<div>')`, `.addClass()`, `.css()`, `.append()`, `area.children().length`, etc.) for rendering high-frequency streaming data like logs causes significant allocation overhead. Instantiating a new jQuery wrapper and invoking its abstraction layer per stream chunk can take ~46ms per batch compared to ~0.8ms for raw DOM nodes, significantly degrading the main thread's performance. Also, `area.children().length` forces unnecessary Array/NodeList allocations compared to the native `area.childElementCount`.
**Action:** When working on performance-critical log or data streaming UI functions (e.g., `appendLogLine`), bypass jQuery and use native DOM creation and modification APIs (`document.createElement()`, `div.className`, `div.style.cssText`, `div.innerHTML`, `area.appendChild()`, and `area.childElementCount`). Keep `$.data(div, 'key', val)` directly on the native element to preserve integration with other jQuery code without needing full jQuery wrapper allocations.
